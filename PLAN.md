...
- [ ] **1.1 Next.js Setup:** Initialize `package.json` with Next.js 15, React 19, and required dependencies.
- [ ] **1.2 Database Infrastructure:** Create `docker-compose.yml` for PostgreSQL 16 and apply initial Prisma migrations (`npx prisma migrate dev`).
...
- [ ] **2.1 Income API & UI:** Implement `/api/income` to record payments. Enforce logic to track both full and partial payments.
- [ ] **2.2 Expense API & UI:** Implement `/api/expenses` with approval status workflow.
...
- [ ] **3.1 Add Fund System:** Implement API and UI for adding non-shareable funds and owner contributions.
...
- [ ] **5.1 Overview Dashboard:** Aggregate metrics for total cash flow, reserve balance, and outstanding loans.

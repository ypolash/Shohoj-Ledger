...
/**
 * PurchaseDetailDrawer (Version 2.5)
 * Slide-over inspector for purchase order details, supplier info, line items, discounts, and payments.
 */
export default function PurchaseDetailDrawer({
  ...
  const shippingAmount = Number(purchase.shippingAmount || 0);
  const totalAmount = Number(purchase.totalAmount || (subtotal - discountAmount + shippingAmount) || 0);
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "24px", color: "var(--text-main)" }}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px", textAlign: "left" }}>
                ...
                <tbody>
                  {(purchase.lines || []).map((line: any, idx: number) => {
                    const itemName = line.product?.name || line.remarks || line.productId || "Product Item";
                    const itemCode = line.product?.productCode || line.product?.sku || "";
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: "13px" }}>
              ...
              <span style={{ fontWeight: 600, fontFamily: "monospace" }}>৳{subtotal.toLocaleString()}</span>
            </div>
            {discountAmount > 0 && (
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: "13px", color: "#ef4444" }}>
                <span>Discount Applied:</span>
                <span style={{ fontWeight: 700, fontFamily: "monospace" }}>-৳{discountAmount.toLocaleString()}</span>
              </div>
            )}
            {shippingAmount > 0 && (
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: "13px" }}>
                <span style={{ color: "var(--text-muted)" }}>Shipping / Freight:</span>
                <span style={{ fontWeight: 600, fontFamily: "monospace" }}>+৳{shippingAmount.toLocaleString()}</span>
              </div>
            )}
            <div
              style={{
            ...
            >
              <span style={{ color: "var(--text-muted)", fontWeight: 600 }}>Total Expense Recognized:</span>
              <span style={{ fontWeight: 800, fontFamily: "monospace", color: "var(--text-main)", fontSize: "15px" }}>

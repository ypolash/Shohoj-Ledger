...
export default function NewQuotationPage() {
  ...
  const [lines, setLines] = useState<any[]>([
    { productId: "", description: "", quantity: 1, unitPrice: 0, discountPercent: 0, taxPercent: 0, lineTotal: 0 }
  ]);
  ...
  useEffect(() => {
    ...
    ]).then(([custData, oppData, prodData, empData]) => {
  ...
  const addLine = () => {
    setLines([...lines, { productId: "", description: "", quantity: 1, unitPrice: 0, discountPercent: 0, taxPercent: 0, lineTotal: 0 }]);
  };
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push(`/dashboard/crm/quotations/${data.id}`);
      } else {
    ...
    <form onSubmit={handleSubmit} className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex justify-between items-center bg-white p-4 rounded shadow">
        ...
        <div className="space-x-2">
          <Link href="/dashboard/crm/quotations" className="px-4 py-2 border rounded text-gray-600 hover:bg-gray-50">Cancel</Link>
          ...
          <div className="bg-white p-6 rounded shadow space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input required type="date" className="w-full border p-2 rounded focus:ring focus:border-blue-300" value={quotationDate} onChange={e => setQuotationDate(e.target.value)} />
              </div>
              <div>
          ...
          <div className="bg-white p-6 rounded shadow space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <textarea className="w-full border p-2 rounded h-24 text-sm" placeholder="Dear Customer, thank you for..." value={customerNotes} onChange={e => setCustomerNotes(e.target.value)}></textarea>
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Internal Notes (Hidden)</label>
                <textarea className="w-full border p-2 rounded h-24 text-sm" placeholder="Internal remarks..." value={internalNotes} onChange={e => setInternalNotes(e.target.value)}></textarea>
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Payment Terms</label>
                <textarea className="w-full border p-2 rounded h-16 text-sm" placeholder="e.g. 50% Advance, 50% on Delivery" value={paymentTerms} onChange={e => setPaymentTerms(e.target.value)}></textarea>
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Delivery Terms</label>
                <textarea className="w-full border p-2 rounded h-16 text-sm" placeholder="e.g. Delivered within 30 days" value={deliveryTerms} onChange={e => setDeliveryTerms(e.target.value)}></textarea>
              </div>
              <div className="md:col-span-2">
                <label className="block text-sm font-medium mb-1">Shipping Terms</label>
                <textarea className="w-full border p-2 rounded h-16 text-sm" placeholder="e.g. FOB, CIF..." value={shippingTerms} onChange={e => setShippingTerms(e.target.value)}></textarea>
              </div>
            </div>
          </div>
        </div>

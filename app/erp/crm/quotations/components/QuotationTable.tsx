          ...
          {quotations.map((quote) => (
            <tr key={quote.id} style={{ borderBottom: '1px solid var(--border-light)', transition: 'background var(--transition-fast)' }} className="hover-row">
              <td style={{ padding: '16px 24px' }}>
                <Link href={`/erp/crm/quotations/${quote.id}`} style={{ fontWeight: 600, color: 'var(--primary)', textDecoration: 'none' }}>
              ...
              <td style={{ padding: '16px 24px', textAlign: 'right' }}>
                <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                  <Link href={`/erp/crm/quotations/preview?id=${quote.id}`}>
                  ...
                  <Link href={`/erp/crm/quotations/${quote.id}/edit`}>

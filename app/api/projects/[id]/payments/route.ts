...
export async function POST(req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx: any) => {
      ...
      // Update actual incurred cost on project
      ...
      await tx.projectActivity.create({
        data: {
          ...
          description: `Recorded payment of ৳${paymentAmount.toLocaleString()} (${paymentMethod}). Auto-paid staff: ৳${staffPayoutTotal.toLocaleString()}, Net Profit: ৳${profit.toLocaleString()}`,

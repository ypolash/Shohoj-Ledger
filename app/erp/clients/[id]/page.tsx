    ...
    <div className="animate-fade-in container">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: "var(--spacing-6)" }}>
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          ...
          <button className="btn btn-secondary" onClick={() => router.push("/erp/clients/list")}>
            Back
          </button>
        </div>
      </div>
                ...
                <tbody>
                  {client.projects.map((p: any) => (
                    <tr key={p.id} style={{ borderBottom: '1px solid var(--border)' }}>
                      ...
                      <td style={{ padding: '12px' }}><Link href={`/erp/projects/${p.id}`} style={{ color: 'var(--primary)' }}>View Project</Link></td>
                    </tr>
                  ))}
                </tbody>
              </table>

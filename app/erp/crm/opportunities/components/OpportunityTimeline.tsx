...
export function OpportunityTimeline() {
  const activities = [
    { id: 1, type: 'STAGE_CHANGE', title: 'Moved to Negotiation', date: new Date().toISOString(), detail: 'Probability updated to 75%' },
    { id: 2, type: 'MEETING', title: 'Product Demo', date: new Date(Date.now() - 86400000).toISOString(), detail: 'Feedback was very positive.' },
    { id: 3, type: 'CREATED', title: 'Opportunity Identified', date: new Date(Date.now() - 86400000 * 5).toISOString() },
  ];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', padding: '16px 0' }}>
      {activities.map((act, index) => {
        ...
        if (act.type === 'STAGE_CHANGE') { icon = 'swap_horiz'; color = 'var(--warning)'; }

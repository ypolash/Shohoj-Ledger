...
export function CustomerSearch({ onSearch, initialValue = "" }: CustomerSearchProps) {
  ...
  // Sync when initialValue explicitly changes externally
  useEffect(() => {
    setQuery(initialValue);
  }, [initialValue]);

  // Debounced search trigger ONLY when query changes by user typing
  useEffect(() => {
  ...
  return (
    <div style={{ position: 'relative', flex: '1 1 280px', maxWidth: '440px' }}>
      ...
      <input 
        ref={inputRef}
        type="text" 
        placeholder="Search by name, customer code, email, phone..."
        value={query}

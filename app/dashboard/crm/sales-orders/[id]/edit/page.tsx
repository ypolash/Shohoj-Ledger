...
export default function EditSalesOrderPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch(`/api/crm/sales-orders/${params.id}`).then(r => r.json())
    ]).then(([custData, oppData, prodData, whData, soData]) => {
      ...
      if (soData && !soData.error) {
        setCustomerId(soData.customerId || "");
        setOrderDate(soData.orderDate ? new Date(soData.orderDate).toISOString().split('T')[0] : "");
        setRequestedDeliveryDate(soData.requestedDeliveryDate ? new Date(soData.requestedDeliveryDate).toISOString().split('T')[0] : "");
        setCurrency(soData.currency || "BDT");
        
        setGlobalDiscount(Number(soData.discountAmount) || 0);
        setShippingAmount(Number(soData.shippingAmount) || 0);
        
        setLines(soData.lines.map((l: any) => ({
          id: l.id,
        ...
        if (soData.remarks) {
          try {
            const parsed = JSON.parse(soData.remarks);
            ...
            setCustomerNotes(soData.remarks);
          }
        }
      }
      setLoading(false);
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      if (res.ok) {
        router.push(`/dashboard/crm/sales-orders/${params.id}`);
      } else {
    ...
    <form onSubmit={handleSubmit} className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex justify-between items-center bg-white p-4 rounded shadow">
        ...
        <div className="space-x-2">
          <Link href={`/dashboard/crm/sales-orders/${params.id}`} className="px-4 py-2 border rounded text-gray-600 hover:bg-gray-50">Cancel</Link>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white p-6 rounded shadow space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Order Date (Read Only)</label>
                <input disabled type="date" className="w-full border p-2 rounded bg-gray-100" value={orderDate} />
              </div>
              <div>

          ...
          {orders.map((order) => (
            <tr key={order.id} style={{ borderBottom: '1px solid var(--border-light)', transition: 'background var(--transition-fast)' }} className="hover-row">
              <td style={{ padding: '16px 24px' }}>
                <Link href={`/dashboard/crm/sales-orders/${order.id}`} style={{ fontWeight: 600, color: 'var(--primary)', textDecoration: 'none' }}>
              ...
              <td style={{ padding: '16px 24px', textAlign: 'right' }}>
                <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                  <Link href={`/dashboard/crm/sales-orders/${order.id}/edit`}>

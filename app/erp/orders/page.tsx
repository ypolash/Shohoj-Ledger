...
export default function StandaloneOrdersPage() {
  ...
  const grossRevenue = useMemo(() => {
    return orders.reduce((sum, o) => sum + Number(o.grandTotal || o.totalAmount || o.total || 0), 0);
  }, [orders]);

  const fulfilledCount = useMemo(() => {
    return orders.filter(o => ['Completed', 'Delivered', 'APPROVED', 'Fulfilled', 'DELIVERED'].includes(o.status)).length;
  }, [orders]);

  const pendingCount = useMemo(() => {
    return orders.filter(o => !['Completed', 'Delivered', 'APPROVED', 'Fulfilled', 'DELIVERED', 'Cancelled', 'CANCELLED'].includes(o.status)).length;
  }, [orders]);

  // Filtered Orders
  const filteredOrders = useMemo(() => {
    return orders.filter(order => {
      const orderNo = (order.salesOrderNumber || order.orderNumber || order.orderNo || order.id || '').toLowerCase();
      const custName = (order.customer?.displayName || order.customer?.name || order.customer?.customerName || '').toLowerCase();
      const matchesSearch = !search.trim() || orderNo.includes(search.toLowerCase()) || custName.includes(search.toLowerCase());
      
      let matchesStatus = true;
      if (statusFilter !== 'ALL') {
        const sUpper = (order.status || '').toUpperCase();
        if (statusFilter === 'Confirmed') {
          matchesStatus = sUpper.includes('CONFIRM');
        } else if (statusFilter === 'Approved') {
          matchesStatus = sUpper.includes('APPROV');
        } else if (statusFilter === 'Open') {
          matchesStatus = sUpper.includes('OPEN') || sUpper.includes('DRAFT') || sUpper.includes('PROCESS');
        } else if (statusFilter === 'Delivered') {
          matchesStatus = sUpper.includes('DELIVER') || sUpper.includes('COMPLET') || sUpper.includes('FULFILL');
        } else if (statusFilter === 'Cancelled') {
          matchesStatus = sUpper.includes('CANCEL');
        } else {
  ...
  const handleExportCSV = () => {
    ...
    const data = filteredOrders.map(o => ({
      OrderNumber: o.salesOrderNumber || o.orderNumber || o.orderNo || o.id,
      Date: new Date(o.orderDate || o.createdAt).toLocaleDateString(),
      Customer: o.customer?.displayName || o.customer?.name || o.customer?.customerName || 'Walk-in Customer',
      ItemsCount: o.lines?.length || o.items?.length || 1,
      GrandTotal: Number(o.grandTotal || o.totalAmount || o.total || 0),
    ...
    <div className={styles.pageContainer}>
      ...
      <div className={styles.headerWrapper}>
        ...
        <div className={styles.actionGroup}>
          ...
          <Link href="/erp/inventory/orders/new?from=/erp/orders" className={styles.btnPrimary}>
            <Plus size={16} />
      ...
      <div className={styles.kpiGrid}>
        <div className={styles.kpiCard}>
          ...
          <div className={styles.kpiContent}>
            ...
            <span className={styles.kpiSubtitle}>All active customer records</span>
          </div>
        </div>
        ...
        <div className={styles.kpiCard}>
          ...
          <div className={styles.kpiContent}>
            ...
            <span className={styles.kpiValue}>৳{grossRevenue.toLocaleString()}</span>
            <span className={styles.kpiSubtitle}>Cumulative order value</span>
          </div>
        </div>

        <div className={styles.kpiCard}>
          <div className={styles.kpiIcon} style={{ background: 'rgba(99, 102, 241, 0.12)', color: '#6366f1' }}>
            <CheckCircle2 size={24} />
          </div>
          <div className={styles.kpiContent}>
            <span className={styles.kpiTitle}>Fulfilled Orders</span>
            <span className={styles.kpiValue}>{fulfilledCount}</span>
            <span className={styles.kpiSubtitle}>Successfully delivered</span>
          </div>
        </div>
        ...
        <div className={styles.kpiCard}>
          ...
          <div className={styles.kpiContent}>
            <span className={styles.kpiTitle}>Pending Fulfillment</span>
            <span className={styles.kpiValue}>{pendingCount}</span>
            <span className={styles.kpiSubtitle}>Processing or awaiting dispatch</span>
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className={styles.tablePanel} style={{ padding: '48px 24px', textAlign: 'center' }}>
          ...
          <Link href="/erp/inventory/orders/new?from=/erp/orders" className={styles.btnPrimary} style={{ marginTop: '16px' }}>
            ...
            <table className={styles.table}>
              ...
              <tbody>
                {filteredOrders.map(order => {
                  const sUpper = (order.status || '').toUpperCase();
                  const isDelivered = ['COMPLETED', 'DELIVERED', 'APPROVED', 'FULFILLED'].includes(sUpper);
                  const isCancelled = sUpper.includes('CANCEL');
                  const orderNumber = order.salesOrderNumber || order.orderNumber || order.orderNo || order.id.slice(0, 8);
                  const customerName = order.customer?.displayName || order.customer?.name || order.customer?.customerName || 'Walk-in Customer';
                  const grandTotal = Number(order.grandTotal || order.totalAmount || order.total || 0);
                  const itemsCount = order.lines?.length || order.items?.length || 1;
